...
# PL 202204:	update v3
# 		- maximum number of slices increased to 99
#		to do: remove unnecessary outputs so this script can be run in the background 
#		("./data_compression_v3.sh &")
#
...
Example usage: ./data_compression_v3.sh -s 1 -w 1 -o \$PWD\n"
			exit 2
			;;
	esac
done

[ $data_compression_spectra == 0 ] && [ $data_compression_water_spectra == 0 ] && exit 0 # echo "No flags set for data_compression_v2.sh! Not compressing anything." && exit 0


# echo -e "\nCompressing data into diamonds (now in parallel :D). \n"

# If out_dir is not set:
[ -z ${out_dir} ] && out_dir=$PWD # && echo "Variable out_dir is empty! Using current directory."
...
######## Moving spectra in subfolders based on slice 
		...
		for i in {01..99}							# Hardcoded to our resolution (and again below!)
		do
		   	# printf "Slice $i: "
...
######## Packing spectra 		
		...
		cd ${out_dir}
		[ "$(ls -A $out_dir/Compressed_Spectra)" ] && rm -R ./spectra # && echo "Removing spectra folder" || echo "Compressed_Spectra appears to be emtpy! I will keep the uncompressed files for now."
...
######## Packing spectra 
		...
		cd ${out_dir}
		[ "$(ls -A $out_dir/Compressed_Water_Spectra)" ] && rm -R ./water_spectra # && echo "Removing water_spectra folder"  || echo "Compressed_Water_Spectra appears to be emtpy! I will keep the uncompressed files for now."					
	fi
fi
timerend2=`date +%s`

cd $currentdirectory
# echo "data_compression_v3.sh: Diamonds generated! This process took around "$((((timerend1-timerstart))/60))" minutes for the metabolite spectra and "$((((timerend2-timerend1))/60))" minutes for the water spectra."
